...
setDT(il6_10_data)
il6_10_data[logCPM >1 & FDR < 0.05 & logFC < -1] |> nrow()
il6_10_data[logCPM >1 & FDR < 0.05 & logFC > 1] |> nrow()
```
...
# Legend numbers
category_counts <- table(il6_10_data$diffexpressed)
...
plot_volcano_il6_10 <-
  ...
  geom_text_repel(
    ...
    # box.padding = 0.5,
    # force = 1,
    show.legend = FALSE # Dont let legend inherit symbols from this function. They look weird
  ) +
  scale_color_manual(
    values = c("blue", "black", "red"), # Custom colors for differential expression
    labels = labels_with_counts
  ) + # Custom labels w/ counts differential expression
  ...
  geom_point() + # Add points for each gene
  labs(
    color = bquote(bold("Direction of gene regulation")), # Customize legend title
    x = bquote(bold("log"[2] * "FC")), # Customize x-axis label
    y = bquote(bold("-log"[10] * "FDR")) # Customize y-axis label
  ) +
...
ggsave(filename = here("notebooks/seq/output/IL_6_10ng_volcano.pdf"),
       device = "pdf",
       width = 8.27,
       height = 11.69,
       units = "in", )
...
setDT(il6_0.1_data)
il6_0.1_data[logCPM > 1 & FDR < 0.05] |> nrow()
# Only 0/2 sig diff genes  with logcpm > 1, FDR >0.05, and logFC +1/-1, respectively:
il6_0.1_data[logCPM > 1 & FDR < 0.05 & logFC > 1] |> nrow()
il6_0.1_data[logCPM > 1 & FDR < 0.05 & logFC < -1] |> nrow()
# 5/4 genes when log FC +0.5/-0.5
il6_0.1_data[logCPM > 1 & FDR < 0.05 & logFC > 0.5] |> nrow()
il6_0.1_data[logCPM > 1 & FDR < 0.05 & logFC < -0.5] |> nrow()
# 25/8 for logFC any 
...
# Legend numbers
category_counts <- table(il6_0.1_data$diffexpressed)
labels_with_counts <- paste(names(category_counts), ": ", category_counts, " genes", sep = "")

# Create the volcano plot using ggplot2
plot_volcano_il6_0.1 <- 
  ggplot(data=il6_0.1_data,
         aes(x=logFC,
             y=-log10(FDR),
             col=diffexpressed,
             label=delabel
    ...
    geom_hline(yintercept=-log10(0.05), col="dark grey", linetype = "dashed") + # Horizontal line for FDR threshold
    geom_point() + # Add points for each gene
    labs(color = bquote(bold("Direction of gene regulation")), # Customize legend title
         x = bquote(bold("log"[2]*"FC")), # Customize x-axis label
         y = bquote(bold("-log"[10]*"FDR")) # Customize y-axis label
         ) +
    #scale_x_continuous(limits = c(-8, 10)) + # Customize x-axis breaks
...
#        height = 11.69,
#        units= "in")
# ggsave(filename = here("notebooks/seq/output/IL_6_01ng_volcano.pdf"),
#        device = "pdf",
#        width = 8.27,
#        height = 11.69,
#        units= "in")
...
# Fix pathway names:
GO_processed_data <- map(GO_processed_data, function(df) {
  df %>%
    mutate(
      pathway = str_remove(pathway, "^GO(BP|MF|CC)_"), 
...
Rerun the GO split databases 
```{r}
# Fix the pathway column string naming the pathways, to look better. Dataframes are already processed form earlier
GO_BP_pathway_names <- GO_processed_data

GO_BP_pathway_names <- map(GO_BP_pathway_names, function(df) {
  df %>%
    mutate(
...
### IL1-B 
Vulcano plots already saved in output. 
Here call and save the needed GSEA plots.
...
# GO_plots$`IL6 0.1`
# 
# ggsave(filename = here("notebooks/seq/output/GSEA_il6_01.png"),
#        device = "png",
#        width = 8.27,
#        height = 11.69,
#        units= "in")
# ggsave(filename = here("notebooks/seq/output/GSEA_il6_01.pdf"),
#        device = "pdf",
#        width = 8.27,
#        height = 11.69,
#        units= "in")
...
# GO_plots$`IL6 10`$BP
# 
# ggsave(filename = here("notebooks/seq/output/GSEA_il610.png"),
#        device = "png",
#        width = 8.27,
#        height = 11.69,
#        units= "in")
# ggsave(filename = here("notebooks/seq/output/GSEA_il6_10.pdf"),
#        device = "pdf",
#        width = 8.27,
#        height = 11.69,
#        units= "in")
